...
function renderRows(result) {
  ...
  for (const row of result.rankings.slice(0, 20)) {
    ...
    tr.innerHTML = `
      ...
      <td>${usd(row.feesEarnedUsd || 0, 2).replace('+', '')}</td>
...
async function lookupWallet() {
  ...
  try {
    ...
    el.walletResult.innerHTML = `
      ...
      Current LP: ${usd(row.currentPositionUsd, 2).replace('+', '')} · Fees: ${usd(row.feesEarnedUsd || 0, 2).replace('+', '')} · Positions: ${row.positionCount}
    `;
    setStatus('');
  } catch (error) {
    setStatus(error.message, true);
